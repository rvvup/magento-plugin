...
test("Can place an inline pay by card order with 3DS challenge", async ({
  page,
}) => {
...
test("Can place an inline pay by card order without 3DS challenge", async ({
  page,
}) => {
  await new VisitCheckoutPayment(page).visit();
...
test("Cannot place pay by card order using invalid card details", async ({
  page,
}) => {
  await new VisitCheckoutPayment(page).visit();
