...
test("Clearpay infographic updates when total changes on checkout page", async ({
  page,
}) => {
  ...
  await expect(await checkoutPage.getGrandTotalElement()).not.toHaveText(
    initialGrandTotal,
  );

  const currentGrandTotal = await checkoutPage.getGrandTotalValue(true);
  let clearpayPayBy4Amount = (
    Math.round((parseFloat(currentGrandTotal) / 4) * 100) / 100
  ).toString();
...
test("Pay by bank infographic is loaded", async ({ page }) => {
  ...
  await expect(
    await page.locator("#placeholder-payment-iframe-rvvup_YAPILY"),
