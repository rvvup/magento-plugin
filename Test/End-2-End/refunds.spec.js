...
test("full refund", async ({
    ...
    await expect(page.locator('.message-success')).toContainText('You created the credit memo.');
});
...
test("partial refund", async ({
    ...
    await expect(page.getByRole('button', { name: 'Update Totals' })).toBeEnabled();
    await page.getByRole('button', { name: 'Update Totals' }).click();
    await expect(page.getByRole('button', { name: 'Update Totals' })).toBeDisabled();
    await page.getByRole('button', { name: 'Refund', exact: true }).click();
    await expect(page.locator('.message-success')).toContainText('You created the credit memo.');
});
