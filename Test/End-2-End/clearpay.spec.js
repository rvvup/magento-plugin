...
test("Renders the Clearpay widget on the product page", async ({ page }) => {
  ...
  await expect(
    page.getByRole("button", { name: "Clearpay logo - Opens a dialog" }),
  ).toBeVisible();

  await page
    .getByRole("button", { name: "Clearpay logo - Opens a dialog" })
    .click();
...
test.skip("Clearpay not available for restricted products", async ({
  page,
}) => {
  await new GoTo(page).product.standard();
  ...
  await expect(
    page.getByText(
      "This item has restrictions so not all payment methods may be available",
    ),
  ).toBeVisible();
...
test.skip("Clearpay not available for products below price threshold", async ({
  page,
}) => {
  await new GoTo(page).product.standard();
...
test.skip("Clearpay not available for products above price threshold", async ({
  page,
}) => {
  await new GoTo(page).product.standard();
...
test("Clearpay shows correct instalment amounts on product page", async ({
  page,
}) => {
  await new GoTo(page).product.standard();
  await expect(
    page.getByText("or 4 interest-free payments of £1.75 with"),
  ).toBeVisible();
});
``;
